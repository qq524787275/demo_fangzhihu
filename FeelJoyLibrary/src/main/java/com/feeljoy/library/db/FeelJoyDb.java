package com.feeljoy.library.db;
...
import com.feeljoy.library.db.sqlite.CursorUtils;
import com.feeljoy.library.db.sqlite.DbModel;
import com.feeljoy.library.db.sqlite.ManyToOneLazyLoader;
import com.feeljoy.library.db.sqlite.OneToManyLazyLoader;
import com.feeljoy.library.db.sqlite.SqlBuilder;
import com.feeljoy.library.db.sqlite.SqlInfo;
import com.feeljoy.library.db.table.KeyValue;
import com.feeljoy.library.db.table.ManyToOne;
import com.feeljoy.library.db.table.OneToMany;
import com.feeljoy.library.db.table.TableInfo;
...
public class FeelJoyDb {

	private static final String TAG = "FeelJoyDb";

	private static HashMap<String, FeelJoyDb> daoMap = new HashMap<String, FeelJoyDb>();

	private SQLiteDatabase db;
	private DaoConfig config;

	private FeelJoyDb(DaoConfig config) {
		if (config == null)
	...
	private synchronized static FeelJoyDb getInstance(DaoConfig daoConfig) {
		FeelJoyDb dao = daoMap.get(daoConfig.getDbName());
		if (dao == null) {
			dao = new FeelJoyDb(daoConfig);
	 ...
	 * @param context
	 */
	public static FeelJoyDb create(Context context) {
	 ...
	 */
	public static FeelJoyDb create(Context context, boolean isDebug) {
		DaoConfig config = new DaoConfig();
		config.setContext(context);
	 ...
	 *            数据库名称
	 */
	public static FeelJoyDb create(Context context, String dbName) {
		DaoConfig config = new DaoConfig();
		config.setContext(context);
	 ...
	 */
	public static FeelJoyDb create(Context context, String dbName, boolean isDebug) {
		DaoConfig config = new DaoConfig();
		config.setContext(context);
		config.setDbName(dbName);
		config.setDebug(isDebug);
	 ...
	 *            数据库名称
	 */
	public static FeelJoyDb create(Context context, String targetDirectory,
								   String dbName) {
		DaoConfig config = new DaoConfig();
		config.setContext(context);
		config.setDbName(dbName);
	 ...
	 * @return
	 */
	public static FeelJoyDb create(DaoConfig daoConfig) {
	 ...
	 *            条件为空的时候，将会更新所有的数据
	 */
	public synchronized void update(Object entity, String strWhere) {
		checkTableExist(entity.getClass());
	 ...
	 *            主键值
	 */
	public synchronized  void deleteById(Class<?> clazz, Object id) {
	 ...
	 *            条件为空的时候 将会删除所有的数据
	 */
	public synchronized  void deleteByWhere(Class<?> clazz, String strWhere) {
		checkTableExist(clazz);
	 ...
	 * @param clazz
	 */
	public synchronized void dropTable(Class<?> clazz) {
		checkTableExist(clazz);
