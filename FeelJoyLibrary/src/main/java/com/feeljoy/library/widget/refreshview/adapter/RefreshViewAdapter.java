...
import com.feeljoy.library.widget.refreshview.RefreshRecyclerView;
 ...
 */
public abstract class RefreshViewAdapter<VH extends RecyclerView.ViewHolder> extends RecyclerView.Adapter<VH> implements StickyRecyclerHeadersAdapter<RecyclerView.ViewHolder>, ItemTouchHelperAdapter {
    protected Handler timer = new Handler();
    protected RefreshRecyclerView.CustomRelativeWrapper customHeaderView = null;
     ...
     */
    public void setCustomHeaderView(RefreshRecyclerView.CustomRelativeWrapper customHeaderView) {
        this.customHeaderView = customHeaderView;
        customHeader = true;
    }

    public RefreshRecyclerView.CustomRelativeWrapper getCustomHeaderView() {
    ...
    protected boolean detectDispatchLoadMoreDisplay(final int data_size_before_remove, final int display_size_before_remove) {
        if (data_size_before_remove == 0) {
            if (display_size_before_remove == 2) {
                ...
                if (mEmptyViewPolicy == RefreshRecyclerView.EMPTY_KEEP_HEADER_AND_LOARMORE) {

                } else if (mEmptyViewPolicy == RefreshRecyclerView.EMPTY_KEEP_HEADER) {
                    removeDispatchLoadMoreView();
                } else if (mEmptyViewPolicy == RefreshRecyclerView.EMPTY_CLEAR_ALL) {
                    removeDispatchLoadMoreView();
                }
                ...
                if (mEmptyViewPolicy == RefreshRecyclerView.EMPTY_KEEP_HEADER_AND_LOARMORE) {
                    ...
                    notifyDataSetChanged();
                } else if (mEmptyViewPolicy == RefreshRecyclerView.EMPTY_SHOW_LOADMORE_ONLY) {
                    notifyDataSetChanged();
                }
                return true;
            } else {
